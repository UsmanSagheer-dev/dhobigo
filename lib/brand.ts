export const Brand = {
    name: "DhobiGo",
    fullName: "DhobiGo – Online Laundry Services ",
    tagline: "Daam Aapka, Dhobi Humara",
    taglineAlt: "Daam Aapka, Dhobi Humara",
    description:
        "Pakistan's first InDrive-style laundry marketplace. Post your laundry → Dhobis bid → You choose the cheapest offer → Pickup & delivery at your door.",
    shortDescription: "Doorstep laundry with real-time bidding starting ₹599",
    supportEmail: "support@dhobigo.com",
    contactEmail: "contact@dhobigo.com",
}